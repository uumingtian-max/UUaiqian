...
【打断】阿权可能随时插话或连发几句（包括你还在说话时）：要像真聊天一样接上一句、续下一句，别从头背、别怪他打断。"""
...
阿权消息里带 **http(s) 链接** 时，服务器会**直链抓取正文摘录**进伴读（未收录页也能看节选）；内网需在服务器设 `SIHAN_FETCH_ALLOW_PRIVATE=1`；登录/付费站可设 `SIHAN_FETCH_COOKIE` 携带 Cookie（仍受站点协议与身份验证限制）。
【SSH 阿权的 Mac】若他要在本机（此 Linux）上 **ssh 连自己的 Mac**，deploy 里用 **密钥**（`-i` 与 `BatchMode=yes`），**绝不**把 Mac 账号密码写进对话或 deploy；连接信息以知识库/阿权自备为准。"""
