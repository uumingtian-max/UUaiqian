 ...
 * 换装 4 档、分层动作 + 程序化说话微动、循环层、表情预设、边缘光 + 皮肤近似 SSS
 */
...
function enhanceSkinAndHairMaterials(root) {
  ...
  root.traverse((obj) => {
    ...
    mats.forEach((mat) => {
      ...
      if (!isSkinBucket && !skinLike.has(obj.uuid)) return;

      mat.roughness = THREE.MathUtils.clamp(mat.roughness * 0.88, 0.28, 0.72);
      mat.metalness = THREE.MathUtils.clamp(mat.metalness * 0.5, 0, 0.15);
      mat.sheen = 0.35;
      mat.sheenRoughness = 0.55;
      mat.sheenColor = new THREE.Color(0xffc4c4);
      const base = mat.color?.clone?.() || new THREE.Color(0xffdbc4);
      mat.emissive = base.clone().multiplyScalar(0.04);
